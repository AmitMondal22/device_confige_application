    ...
    public partial class Dashboard : Form
    {
        ...
        private void btnConnect_Click(object sender, EventArgs e)
        {
            try
            {
                if (IsConnected == 0)
                {
                    port = new SerialPort(cmbComPorts.SelectedItem.ToString(), 115200, Parity.None, 8, StopBits.One);
        ...
        private void DoUpDate(object s, EventArgs e)
        {
            try
            {
                ...
                if (IsConnected == 0)
                    return;
                //Thread.Sleep(500);
                //await Task.Delay(500); // Pause for 500ms without blocking UI
                string newData = port.ReadExisting(); // Read available serial data
                ...
                if (!string.IsNullOrEmpty(newData))
                {
                    ...
                    else if (newData.Contains("*")&&(packetStarted==false))
                    {
                        stringBuffer = newData.Replace("\r\n", "").Trim(); // Append new data
                        packetStarted = true;
                        Thread.Sleep(100);
                ...
                // Console.WriteLine(incomingDetails);

                //if (string.IsNullOrEmpty(stringBuffer) || stringBuffer.Trim().Length <= 1)
                    //return;
               ...
               // stringBuffer = stringBuffer.Substring(0, stringBuffer.Length);
                ...
                //if (string.IsNullOrEmpty(dataRead) || !stringBuffer.ToUpper().Contains(dataRead.ToUpper()))
                    //return;
                dataRead = string.Empty;
                
                if (stringBuffer.Contains("*") && stringBuffer.Contains("#")) {
                    Debug.WriteLine($"Received Data: {stringBuffer}");
                    int startIndex = stringBuffer.IndexOf('*'); // Find start
                    int endIndex = stringBuffer.LastIndexOf('#'); // Find end
                    string incomingDetails = stringBuffer;
                    stringBuffer = "";
        ...
        private void btnModbusRegisterConnectionReadMemory_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    dataRead = "*REGSET";
                    port.WriteLine("*readdeviceMasterRegisterConnection,"+ cmbMasterSlaveIndex.SelectedIndex + "," + index + "#");
                }
            }
            catch (Exception ex)
            {
